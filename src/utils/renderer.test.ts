...
describe('relative', () => {
  it('should print an absolute path relative to the current working directory', () => {
    expect(
      normalizeWindowsSeperators(relative(path.resolve('index.css'))),
  ...
  it('should prefer the shortest value by default', () => {
    ...
    expect(
      normalizeWindowsSeperators(relative('index.css')),
    ).toMatchInlineSnapshot(`"index.css"`);
  });

  it('should be possible to override the current working directory', () => {
    expect(
      normalizeWindowsSeperators(relative('../utils/index.css', '..')),
    ).toMatchInlineSnapshot(`"./utils/index.css"`);
  });

  it('should be possible to always prefer the relative path', () => {
    expect(
      normalizeWindowsSeperators(
        relative('index.css', process.cwd(), {
          preferAbsoluteIfShorter: false,
        }),
      ),
    ).toMatchInlineSnapshot(`"./index.css"`);
  });
...
describe('word wrap', () => {
  it('should wrap a sentence', () => {
    expect(wordWrap('The quick brown fox jumps over the lazy dog', 10))
    ...
    `);
    expect(wordWrap('The quick brown fox jumps over the lazy dog', 30))
      .toMatchInlineSnapshot(`
      [
